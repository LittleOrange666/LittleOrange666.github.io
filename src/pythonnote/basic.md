...
日期 = (2022, 1, 20)
```
變數名稱可以是任何的文字，可以用任何語言，但通常建議用英文大小寫、數字和底線"\_"
且不能和保留字重複
...
:::spoiler 關於除法
不論用`int`/`float`做`/`總是輸出float
而`//`和`%`則是在有`float`參與計算時輸出`float`

`%`遇到負數會有怪怪行為，要小心

雖然Python有`inf`和`NaN`，但內建計算不會輸出`inf`或`NaN`，會直接報錯
(OverflowError/ZeroDivisionError)

`**`有提供函數的版本，額外有一個"mod"的參數，還可以算模逆元
pow(a,p,m)
:::
...
例外處理可以快速的跳出迴圈/函數，但不建議這樣用(try裡面的所有程式碼都會變慢且可讀性差)
