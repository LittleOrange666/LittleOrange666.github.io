...
bool ok(pos p){return p.x>=0 && p.y>=0 && p.x<m && p.y<n;}
pos add(const pos &a, const pos &b){
    return pos(a.x+b.x,a.y+b.y);
}
void place(int x, int y, int v){
    if (v>0&&stat[x][y]==0) cur++;
    if (v==0&&stat[x][y]>0) cur--;
    stat[x][y] = v;
}
void place(pos p, int v){
    place(p.x,p.y,v);
}
int get(pos p){
    return stat[p.x][p.y];
...
int main(){
    ...
    vector<pos> facings = {pos(1,0),pos(0,1),pos(-1,0),pos(0,-1)};
    for (int z = 0;z<h;z++){
        int r,c,t;
        cin >> r >> c >> t;
        pos p = pos(r,c);
